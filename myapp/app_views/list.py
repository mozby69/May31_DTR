...
def list(request):
    ...
    if request.method == "POST":
        ...
        if "update" in request.POST:
            ...
            messages.success(request, f'{Empname} - {date}', extra_tags='edit')
            return HttpResponseRedirect(request.path)
            ...
            messages.success(request, f'{full_name}', extra_tags='delete_list')
            ...
            try:
                ...
                time_obj_timein = datetime.strptime(formatted_timein, "%H:%M:%S")
                formatted_timein = current_date.replace(hour=time_obj_timein.hour, minute=time_obj_timein.minute, second=time_obj_timein.second).strftime("%Y-%m-%d %H:%M:%S")

                if formatted_breakout is not None:
                    time_obj_breakout = datetime.strptime(formatted_breakout, "%H:%M:%S")
                    formatted_breakout = current_date.replace(hour=time_obj_breakout.hour, minute=time_obj_breakout.minute, second=time_obj_breakout.second).strftime("%Y-%m-%d %H:%M:%S")
                else:
                    formatted_breakout = None
                
                if formatted_breakin is not None:
                    time_obj_breakin = datetime.strptime(formatted_breakin, "%H:%M:%S")
                    formatted_breakin = current_date.replace(hour=time_obj_breakin.hour, minute=time_obj_breakin.minute, second=time_obj_breakin.second).strftime("%Y-%m-%d %H:%M:%S")
                else:
                    formatted_breakin = None

                if formatted_timein is not None and formatted_breakin is not None and formatted_breakout is not None:
                    temporay.objects.create(EmpCode_id=EmpCode,Empname=Empname,date=date,timein_names=EmpCode,breakin_names=EmpCode,breakout_names=EmpCode,timein_timestamps=formatted_timein,breakout_timestamps=formatted_breakout,breakin_timestamps=formatted_breakin)
                
                if formatted_timein is not None and formatted_breakout is None and formatted_breakin is None:
                    temporay.objects.create(EmpCode_id=EmpCode,Empname=Empname,date=date,timein_names=EmpCode,breakin_names=None,breakout_names=None,timein_timestamps=formatted_timein)
                
                if formatted_timein is not None and formatted_breakout is not None and formatted_breakin is None:
                    temporay.objects.create(EmpCode_id=EmpCode,Empname=Empname,date=date,timein_names=EmpCode,breakout_names=EmpCode,breakin_names=None,timein_timestamps=formatted_timein,breakout_timestamps=formatted_breakout)

                messages.success(request, f'{Empname} - {date}', extra_tags='added_list')
...
def fetch_edit_successfully(request):
    ...
    filtered_messages = [
        ...
        or 'added_list' in message.tags or 'delete_list' in message.tags
