...
@login_required(login_url='login')
def index(request):
    branch_names = request.user.username if request.user.is_authenticated else None
...
def display_qr_list(request):
    ...
    if request.user.username == 'ADMIN' or request.user.username == 'MIS_XYRYL':
        ...
        #branches = Employee.objects.values('BranchCode_id') 
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                # elif request.user.username == 'EMB_CADIZ' or request.user.username == 'FCH_SILAY':
